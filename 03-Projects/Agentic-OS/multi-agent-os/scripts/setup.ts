#!/usr/bin/env tsx
/**
 * Setup wizard — first-run config for Multi-Agent OS.
 *
 *   npm run setup
 *
 * What it does:
...
async function main() {
  ...
  ╔══════════════════════════════════════╗
  ║       Multi-Agent OS — Setup Wizard      ║
  ╚══════════════════════════════════════╝${c.reset}`)

  info(`This wizard will configure Multi-Agent OS for your machine.`)
  ...
  if (agents.length === 0) {
    ...
    info('  You can still use Multi-Agent OS — API keys are enough.')
  } else {
