#!/usr/bin/env tsx
/**
 * Setup wizard — first-run config for OpenClaw OS.
 *
 *   npm run setup
 *
 * What it does:
...
async function main() {
  ...
  ╔══════════════════════════════════════╗
  ║       OpenClaw OS — Setup Wizard      ║
  ╚══════════════════════════════════════╝${c.reset}`)

  info(`This wizard will configure OpenClaw OS for your machine.`)
  ...
  if (agents.length === 0) {
    ...
    info('  You can still use OpenClaw OS — API keys are enough.')
  } else {
  ...
  const config: any = {
    app: { name: 'OpenClaw OS', port: 3000, accent: 'cyan' },
    vault: {
      enabled: true,
      localPath: vaultPath,
